	...
	if (pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt - bq_taper_hys_mv) {
		if (ibus_lmt_change_timer++ > IBUS_CHANGE_TIMEOUT) {
			...
			if (pdpm->fcc_votable) {
				...
				pr_err("bq set taper fcc to: %dmA\n", effective_fcc_taper);
			}
		}
...
static void usbpd_pd_contact(struct usbpd_pm *pdpm, bool connected)
{
	...
	if (connected) {
		...
		pr_err("start cp charging, pps_support: %d\n", pdpm->pps_supported);
		if (pdpm->pps_supported)
...
static int __init usbpd_pm_init(void)
{
	...
	pr_err("start, probe_cnt: %d\n", ++probe_cnt);
	if (probe_cnt > 10) {
		pr_err("stop: probe_cnt: %d, max probe_cnt is 10\n", probe_cnt);
		return 0;
	}
	...
	//get master/slave cp psy
	usbpd_check_cp_psy(pdpm);
	if (pm_config.cp_sec_enable) {
		usbpd_check_cp_sec_psy(pdpm);
	}
	//get usb psy, tcpc port
	usbpd_check_charger_psy(pdpm);
	if (NOPMI_CHARGER_IC_MAXIM != nopmi_get_charger_ic_type()) {
		usbpd_check_tcpc(pdpm);
	}
...
static void __exit usbpd_pm_exit(void)
{
	...
	if (pm_config.cp_sec_enable && __pdpm->cp_sec_psy)
		power_supply_put(__pdpm->cp_sec_psy);
	if (__pdpm->usb_psy)
		power_supply_put(__pdpm->usb_psy);
	if (__pdpm->sw_psy)
		power_supply_put(__pdpm->sw_psy);
	if (__pdpm->bms_psy)
		power_supply_put(__pdpm->bms_psy);
